...
要求：
- 正文 HTML 内纯文字（不含标签）不少于 400 字、不超过 550 字，按汉字字符计（模型单次输出上限约 1000 token，请勿超过此长度）；务必在限定长度内完整收尾，结尾必须有 3-5 条可执行行动建议或一句明确行动号召，绝不要停在标题或小节中途；不要靠空话凑字数
...
- 文章分 3 个 h2 章节（建议结构：痛点引入 → 实操干货 → 行动收尾），每节有实质内容，不要再嵌套 h3 子小节
- 严格在 [CONTENT] 标签闭合前写完正文所有内容：确保每个段落都完整闭合、每个章节都有收尾；绝不允许出现未闭合标签、裸标题或停在半句；若篇幅富余可补一句行动建议，但不得以不完整结尾
...
[/CONTENT]`;
  ...
  const body = JSON.stringify({
    ...
    temperature: 0.8,
    top_p: 0.9,
    // 注意：GLM-4V-Flash 的 chat 接口不支持 max_tokens 参数，传入会被 API 拒绝（400），
    // 故不传该参数，改用下方字数要求适配模型默认输出上限防截断。
  });
